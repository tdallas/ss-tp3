...
for n in doorSizeValues:
    cmd = "java -jar target/ss-tp3-1.0.jar -n %s -dt %s -p 0.%s -o output-%s-door -nw" % (str(number_of_particles), str(dt), n, n)
    print(cmd)
    os.system(cmd)

print("Generating animation with no middle wall")
cmd = "java -jar target/ss-tp3-1.0.jar -n %s -dt %s -o output-%s-no-door -nw" % (str(number_of_particles), str(dt), str(number_of_particles))
print(cmd)
os.system(cmd)